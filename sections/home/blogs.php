<section class="w-[90%] mx-auto pb-20">
  <div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
    <div>
      ...
      <h2 class="mt-4 font-display text-2xl font-semibold tracking-tight md:text-4xl">Field notes & guidance.</h2>
      <p class="mt-3 max-w-2xl text-sm leading-relaxed text-white/70">Short reads on investigations, guarding, and reducing operational risk.</p>
    </div>
    <a href="<?php echo url('blog.php'); ?>" class="inline-flex items-center justify-center rounded-full bg-white/5 px-5 py-2.5 text-sm font-semibold text-white/90 ring-1 ring-white/10 transition hover:bg-white/10">View all</a>
  </div>
  ...
  <div class="mt-6 grid gap-3 md:grid-cols-3">
    <?php foreach ($posts as $p) { ?>
      <a data-reveal href="<?php echo url('post.php'); ?>?slug=<?php echo e($p['slug']); ?>" class="group overflow-hidden rounded-3xl bg-white/5 ring-1 ring-white/10 transition hover:-translate-y-0.5 hover:bg-white/10">
        ...
        <div class="p-6">
          <div class="text-sm font-semibold leading-snug group-hover:text-white"><?php echo e($p['title']); ?></div>
          <div class="mt-2 text-xs leading-relaxed text-white/60"><?php echo e($p['excerpt']); ?></div>
          <div class="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-white/70">
            Read
