  ...
  <div class="rounded-[2.5rem] bg-white/5 p-7 ring-1 ring-white/10 md:p-10">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h1 class="mt-5 font-display text-3xl font-semibold tracking-tight md:text-4xl"><?php echo $edit ? 'Edit testimonial' : 'Add testimonial'; ?></h1>
    ...
    <form method="post" class="mt-8 grid gap-5">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <label class="text-xs font-semibold text-white/70">Organization / title</label>
          <input name="org_title" class="mt-2 w-full rounded-2xl bg-zinc-950/40 px-4 py-3 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-brand-300" value="<?php echo e($edit['org_title'] ?? ''); ?>">
        </div>
        <div class="md:col-span-2">
          <label class="text-xs font-semibold text-white/70">Quote</label>
          <textarea name="quote_text" rows="4" required class="mt-2 w-full rounded-2xl bg-zinc-950/40 px-4 py-3 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-brand-300"><?php echo e($edit['quote_text'] ?? ''); ?></textarea>
        </div>
        <div>
          <label class="text-xs font-semibold text-white/70">Rating</label>
          <select name="rating" class="mt-2 w-full rounded-2xl bg-zinc-950/40 px-4 py-3 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-brand-300">
            <?php $r = (int)($edit['rating'] ?? 5); for ($i = 5; $i >= 1; $i--) { ?>
              <option class="bg-zinc-950" value="<?php echo $i; ?>" <?php echo $r === $i ? 'selected' : ''; ?>><?php echo $i; ?></option>
            <?php } ?>
          </select>
        </div>
        <div>
        ...
        <tbody class="divide-y divide-white/10 bg-zinc-950/30">
          <?php foreach ($rows as $t) { ?>
            <tr class="hover:bg-white/5">
              <td class="px-4 py-3">
                <a href="<?php echo url('admin/testimonials.php'); ?>?edit=<?php echo (int)$t['id']; ?>" class="text-white/85 transition hover:text-white"><?php echo e($t['name']); ?></a>
                <div class="text-xs text-white/50"><?php echo e($t['org_title']); ?></div>
              </td>
              <td class="px-4 py-3 text-xs text-white/60"><?php echo e(mb_strimwidth($t['quote_text'], 0, 120, '…')); ?></td>
              <td class="px-4 py-3">
                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ring-1 <?php echo (int)$t['is_active'] === 1 ? 'bg-emerald-300/15 text-emerald-200 ring-emerald-300/20' : 'bg-white/10 text-white/60 ring-white/15'; ?>">
          ...
          <?php } ?>
          <?php if (!$rows) { ?>
            <tr><td class="px-4 py-6 text-sm text-white/60" colspan="4">No testimonials yet.</td></tr>
          <?php } ?>
        </tbody>
      </table>
    </div>
