  ...
  <div class="rounded-[2.5rem] bg-white p-7 ring-1 ring-black/10 md:p-10">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h1 class="mt-5 font-['Sora',ui-sans-serif,system-ui] text-3xl font-semibold tracking-tight md:text-4xl"><?php echo $edit ? 'Edit partner' : 'Add partner'; ?></h1>
        <p class="mt-3 text-sm text-zinc-600">Partner logos appear on the homepage.</p>
      </div>
      <?php if ($edit) { ?>
    ...
    <form method="post" enctype="multipart/form-data" class="mt-8 grid gap-5">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <label class="text-xs font-semibold text-zinc-600">Website</label>
          <input name="website" class="mt-2 w-full rounded-2xl bg-zinc-50 px-4 py-3 text-sm text-zinc-900 ring-1 ring-black/10 focus:outline-none focus:ring-2 focus:ring-sky-300" value="<?php echo e($edit['website'] ?? ''); ?>">
        </div>
        <div>
        ...
        <div class="md:col-span-2">
          <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div class="flex-1">
              <label class="text-xs font-semibold text-zinc-600">Logo path</label>
              <input name="logo" class="mt-2 w-full rounded-2xl bg-zinc-50 px-4 py-3 text-sm text-zinc-900 ring-1 ring-black/10 focus:outline-none focus:ring-2 focus:ring-sky-300" value="<?php echo e($edit['logo'] ?? 'assets/svg/partner-a.svg'); ?>">
            </div>
            <div class="md:w-64">
              <label class="text-xs font-semibold text-zinc-600">Upload logo</label>
              <input type="file" name="logo_upload" accept="image/png,image/jpeg,image/webp,image/svg+xml" class="mt-2 w-full rounded-2xl bg-zinc-50 px-4 py-2 text-sm text-zinc-900 ring-1 ring-black/10">
            </div>
          </div>
          <div class="mt-3 text-xs text-zinc-500">Prefer transparent backgrounds.</div>
        </div>
      </div>
        ...
        <tbody class="divide-y divide-white/10 bg-white/30">
          <?php foreach ($rows as $p) { ?>
            <tr class="hover:bg-white">
              ...
              <td class="px-4 py-3">
                <a href="<?php echo url('admin/partners.php'); ?>?edit=<?php echo (int)$p['id']; ?>" class="text-zinc-800 transition hover:text-indigo-700"><?php echo e($p['name']); ?></a>
                <div class="text-xs text-zinc-500"><?php echo e($p['website']); ?></div>
              </td>
              <td class="px-4 py-3">
                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ring-1 <?php echo (int)$p['is_active'] === 1 ? 'bg-emerald-300/15 text-emerald-700 ring-emerald-300/20' : 'bg-zinc-50 text-zinc-500 ring-black/10'; ?>">
          ...
          <?php } ?>
          <?php if (!$rows) { ?>
            <tr><td class="px-4 py-6 text-sm text-zinc-500" colspan="4">No partners yet.</td></tr>
          <?php } ?>
        </tbody>
      </table>
    </div>
