...
<div class="rounded-[2.5rem] bg-white/5 p-7 ring-1 ring-white/10 md:p-10">
  <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
    <div>
      ...
      <h1 class="mt-5 font-display text-3xl font-semibold tracking-tight md:text-4xl">Global settings</h1>
      <p class="mt-3 text-sm text-white/70">Update the site identity, hero content, mission, and contact details.</p>
    </div>
  ...
  <form method="post" class="mt-8 grid gap-6">
    ...
    <div class="grid gap-4 md:grid-cols-2">
      <?php foreach ($fields as $f) { $k = $f[0]; $label = $f[1]; $type = $f[2]; ?>
        <div class="<?php echo in_array($k, ['hero_title','hero_subtitle','mission','vision','values','cta_subtitle','meta_description'], true) ? 'md:col-span-2' : ''; ?>">
          <label class="text-xs font-semibold text-white/70"><?php echo e($label); ?></label>
          <?php if ($type === 'textarea') { ?>
            <textarea name="<?php echo e($k); ?>" rows="<?php echo in_array($k, ['values'], true) ? 5 : 3; ?>" class="mt-2 w-full rounded-2xl bg-zinc-950/40 px-4 py-3 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-brand-300"><?php echo e(setting($k, '')); ?></textarea>
          <?php } else { ?>
            <input name="<?php echo e($k); ?>" class="mt-2 w-full rounded-2xl bg-zinc-950/40 px-4 py-3 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-brand-300" value="<?php echo e(setting($k, '')); ?>">
          <?php } ?>
        </div>
      <?php } ?>
    </div>
    ...
    <div class="flex items-center justify-between gap-3">
      <div class="text-xs text-white/50">Saved to MySQL and applied instantly on the website.</div>
